<script lang="ts">
  ...
  let { components, h, s = $bindable(), v = $bindable(), isDark, oninput }: Props = $props()
  let picker: HTMLDivElement = $state() as HTMLDivElement
  let isMouseDown = false
  let focused = false
  let pickerColorBg: string = $state() as string
...
<style>
  ...
  .s {
    top: calc(
      var(--pos-y) * (var(--picker-height, 200px) - var(--picker-indicator-size, 10px) - 4px) / 100 +
        2px
    );
    left: 2px;
