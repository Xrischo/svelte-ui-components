<script lang="ts">
  ...
  let { options, value = $bindable(), label = '', showReset = true, error = '' }: Props = $props()

  let initialValue: string | undefined = $state()

  let selectedClass = 'bg-primary-500 text-white dark:bg-primary-600 dark:text-white'
  let initialClass = 'bg-gray-400 text-black dark:bg-gray-500 dark:text-black'
  let unselectedClass = 'bg-surface-200 text-black dark:bg-surface-700 dark:text-white'
  let baseClass = 'w-24 h-8 -skew-x-6'
...
<div class="relative flex flex-col">
  {#if label}
    <label for="linebuttons" class="text-sm ml-1 mb-1">{label}</label>
  {/if}
  <div class="linebuttons">
    {#each options as button}
      <button
        class="{baseClass} {value === button
          ? selectedClass
          : initialValue === button && showReset
            ? initialClass
            : unselectedClass}"
        onclick={() => (value = value === button ? undefined : button)}>{button}</button
      >
    {/each}
    {#if showReset}
      <!-- svelte-ignore a11y_consider_explicit_label -->
      <button disabled={value === initialValue} onclick={reset}>
        <svg
          height="16px"
          viewBox="0 0 512 512"
          class={value === initialValue
            ? 'fill-gray-500 dark:fill-gray-600'
            : 'fill-primary-800 dark:fill-primary-400'}
        >
          <g>
            <path
              class="st0"
